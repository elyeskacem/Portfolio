/* ------------------------------------------------------------------
   ...
   Add a project = add one object below. Fields:
     images : list of imported images, first one = the cover. Several images
              means visitors can sweep across the card to flip through them,
              and the lightbox gets thumbnails:
                images: [SHOT_1, SHOT_2, SHOT_3],
              (put the files in assets/images/ScreenShots)
     title  : card title
     tags   : any of the values in `projectFilters` (first tag = badge)
     disc   : short description, shown on the card and in the lightbox
     demo   : OPTIONAL link (video, live site, repo...). Leave it out and the
              card simply shows no demo button - the images still open full
              size when the card is clicked.
